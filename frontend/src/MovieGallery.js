...
function MovieGallery({ token }) {
  ...
  useEffect(() => {
    ...
    .then(response => {
        console.log("Respuesta de API:", response.data);  // 👈 Aquí vemos lo que devuelve la API
  ...
  return (
    <div className="gallery">
        {movies.length === 0 ? (
            <p style={{ color: 'red' }}>No se encontraron películas o hubo un error al cargarlas.</p>
        ) : (
      movies.map(movie => (
